...
@Component
@Slf4j
public class SaleOrderStockOutPublisher {
    ...
    public void sendMsg(SkuInventoryInOut sio) {
        try {
            ...
            //指定消息模型中的路由
            rabbitTemplate.setRoutingKey(DicConstant.SALE_ORDER_STOCK_OUT_ROUTING_KEY);
            //转化并发送消息
            rabbitTemplate.convertAndSend(JSON.toJSONString(sio));
            log.info("SaleOrderStockOutPublisher-生产者-发送消息：{} ", JSON.toJSONString(sio));
        } catch (Exception e) {
            log.error("SaleOrderStockOutPublisher-生产者-发送消息发生异常：{} ", sio, e.fillInStackTrace());
        }
    }
}
