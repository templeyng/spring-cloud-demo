package com.wisteria.saleServerCenter.message.send;
...
public class SaleOrderDbAddPublisher {
    ...
    public void sendMsg(SaleOrderLd saleOrderLd) {
        try {
            ...
            log.info("rabbitmq demo-生产者-发送消息：{} ", JSON.toJSONString(saleOrderLd));
        } catch (Exception e) {
            log.error("rabbitmq demo-生产者-发送消息发生异常：{} ", saleOrderLd, e.fillInStackTrace());
        }
    }
}
