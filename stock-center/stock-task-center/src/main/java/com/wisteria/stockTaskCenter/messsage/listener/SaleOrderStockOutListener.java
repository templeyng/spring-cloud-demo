...
@Component
@Slf4j
public class SaleOrderStockOutListener {
    ...
    @RabbitListener(queues = DicConstant.SALE_ORDER_STOCK_OUT_QUEUE, containerFactory = "singleListenerContainer")
    public void consumeMsg(String message, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) Long tag) throws IOException {
        try {
            ...
            skuInventoryInOut.setCount(skuInventoryInOut.getCount() * -1);
